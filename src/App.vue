<template>
  <div class="min-h-full">
    <Disclosure as="nav" class="bg-ally-plum-dark" v-slot="{ open }">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center">
            <div class="flex-shrink-0">
              <img class="h-8 w-8" src="/src/assets/ally-logo.png" alt="Your Company" />
            </div>
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-ally-plum text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
              </div>
            </div>
          </div>
          <div class="hidden md:block">
    ...
    <header class="bg-white shadow-sm">
      <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <h1 class="text-lg font-semibold leading-6 text-gray-900">Agile Project Roadmapping Tool</h1>
      </div>
    </header>
    <main>
...
<script setup lang="ts">
  ...
  import { ArrowDownTrayIcon, Bars3Icon, XMarkIcon } from '@heroicons/vue/24/outline'
  import RoadmapTool from './components/RoadmapTool.vue'
  ...
  const navigation = [
    { name: 'Project Roadmap', href: '#', current: true },
  ]
